...
def create_store_file():
    # 存储主页面数据
    file1 = os.getcwd() + '/car16888_multi_page_with_attr_data_pool.csv'
    ...
    # 存储历史销量数据
    file2 = os.getcwd() + '/car16888_data_historical_sales_pool.csv'
    ...
    # 存储城市上牌数据
    file3 = os.getcwd() + '/car16888_data_registered_number_pool.csv'
...
def get_page_info(new_url, file1, file2, file3):
    ...
    pool.submit(get_historical_sales_data,historical_sales_url_dict, file2)
    pool.submit(get_registered_number_data,registered_number_url_dict, file3)
    return result
...
def get_historical_sales_data(dict_historical_sales, file):
    for model, value in dict_historical_sales.items():
        print(datetime.now(), "XXXXX【开始】采集【历史销量】数据,车型=", model)
        ...
        for i in range(1, 10):  # 每行50条数据，450包含10多年的月份了
            ...
            print(datetime.now(), "XXXXX采集【历史销量】数据,url=", new_url)
...
def get_registered_number_data(dict_registered_number, file):
    for model, value in dict_registered_number.items():
        ...
        for i in range(1, 21):  # 每行50条数据，1000大于全国所有的城市数量
            ...
            print(datetime.now(), "######采集【上牌】数量数据,url=", new_url)
            response = requests.get(url=new_url)
            if response.status_code == 200:
                tree = etree.HTML(response.text)
        ...
        with open(file=file, mode="a", encoding="utf-8") as f:
            f.write(data)
        print(datetime.now(), "######【结束】采集【上牌】数量数据,车型=", model)
...
if __name__ == '__main__':
    ...
    pool = ThreadPoolExecutor(max_workers=8)  # 定义最多8个线程的线程池
